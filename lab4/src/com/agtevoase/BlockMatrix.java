...
public class BlockMatrix {
    ...
    public void gaussianBlockSolve(int block) throws Exception {
        ...
        for (int i = 0; i < cBlock.length - 1; ++i) {
            ...
            if (rowMax == -1) {
                ...
                throw new Exception("Block #" + block + ", row #" + i + " seems to be empty");
            }
            if (rowMax != i) {
                swapRaws(cBlock, i, rowMax);
                //System.out.format("Rows %d and %d swapped!\n", i, rowMax);
            }

            for (int j = i + 1; j < cBlock.length; ++j) {
                double multiplier = cBlock[i][i] / cBlock[j][i];
                //System.out.println(Arrays.deepToString(cBlock));
                //System.out.format("Block #%d, row #%d, multiplier=%g\n", block, i, multiplier);
    ...
    //returns true if all blocks are solved
    public boolean nextBlockSolve() throws Exception {
        int localNextBlock;
        synchronized (mutex) {
        ...
        gaussianBlockSolve(localNextBlock);
